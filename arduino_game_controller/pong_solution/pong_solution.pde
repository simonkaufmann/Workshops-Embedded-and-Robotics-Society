...
void serialEvent(Serial myPort) {
  ...
  try {
    ...
    int posAOld = paddleAPos;
    int posBOld = paddleBPos;
    paddleAPos = Integer.parseInt(parts[0]);
    paddleBPos = Integer.parseInt(parts[1]);
    paddleASpeed = (paddleAPos - posAOld) / 3;
    paddleBSpeed = (paddleBPos - posBOld) / 3;
  } catch (Exception ex) {
  }
}
...
void printPaddleA() {
  ...
  fill(255);
  rect(PADDLE_A_POSX, paddleAPos / 2, PADDLE_WIDTH, PADDLE_HEIGHT);
}

void printPaddleB() {
  /* draw rectangle for paddle in correct position */
  fill(255);
  rect(PADDLE_B_POSX, paddleBPos / 2, PADDLE_WIDTH, PADDLE_HEIGHT);
}
